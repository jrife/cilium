...
static __always_inline int mock_get_socket_cookie(void *ctx __maybe_unused)
{
...
static __always_inline int setup(struct sock_term_filter *filter)
{
	...
	return map_update_elem(&cilium_sock_term_filter, &zero, filter, 0);
}
