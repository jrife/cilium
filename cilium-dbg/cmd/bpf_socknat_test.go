...
// TestDumpSKMetaEntries verifies that the CLI tool can correctly display socket metadata entries
func TestDumpReverseSKEntries(t *testing.T) {
	...
	err := mockMap.UpdateSockMeta(1234, net.ParseIP("10.0.2.100"), 80, 1) // IPv4
	if err != nil {
		t.Fatalf("Failed to add IPv4 entry: %v", err)
	}
	err = mockMap.UpdateSockMeta(2345, net.ParseIP("2001:db8::1"), 443, 2) // IPv6
	if err != nil {
	...
	t.Run("IPv4 entry exists", func(t *testing.T) {
		if !mockMap.ExistsSockMeta(1234, net.ParseIP("10.0.2.100"), 80) {
			t.Error("Expected IPv4 entry to exist")
		}
	})

	t.Run("IPv6 entry exists", func(t *testing.T) {
		if !mockMap.ExistsSockMeta(2345, net.ParseIP("2001:db8::1"), 443) {
	...
	t.Run("Non-existent entries", func(t *testing.T) {
		if mockMap.ExistsSockMeta(9999, net.ParseIP("192.168.1.1"), 8080) {
