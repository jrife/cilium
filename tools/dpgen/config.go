...
func fieldsToStruct(fields []field, name string, embeds []string) (string, error) {
	...
	// Render a function that validates config value ranges/sizes and
	// casts values to their original sizes.
...
func camelCase(s string, stylized bool) string {
	var b strings.Builder
...
// btfVarGoType converts the type of an integer btf.Var to its equivalent
// protobuf type name. Some types like char, bool, and integers < 4 bytes
// have no direct representation in protobuf. Instead, we must cast up to the
// next smallest size, (u)int32.
